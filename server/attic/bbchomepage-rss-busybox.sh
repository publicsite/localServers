...
start="$(printf "%s" "${1}" | busybox grep -Eoa "<div data-component=\"${2}.*$")"
counttag=1
...
setit=0
while true; do
	toformata="$(printf "%s" "${toformat}" | cut -d '<' -f 2-)"
	toformatb="$(printf "%s" "${toformat}" | grep -o '/>.*')"
	toformatc="$(printf "%s" "${toformat}" | grep -o '</.*' )"

	if [ "$(printf "%s" "$toformatc" | wc -c)" -gt "$(printf "%s" "$toformatb" | wc -c)" ]; then
		toformatb="$toformatc"	
	fi

	if [ "$(printf "%s" "$toformata" | wc -c)" -eq "$(printf "%s" "$toformatb" | wc -c)" ]; then
		toformata="$(printf "%s" "${toformata}" | cut -d '<' -f 2-)"
	fi

	if [ "$(printf "%s" "$toformata" | wc -c)" -gt "$(printf "%s" "$toformatb" | wc -c)" ]; then
		counttag="$(expr $counttag + 1)"
	else
		counttag="$(expr $counttag - 1)"
	fi

	if [ $counttag -eq 1 ]; then
		toformatd="$(printf "%s" "${toformat}" | cut -d '>' -f 2-)"
		endblock="$(printf "%s" "$toformatd" | wc -c)"
		chopoff="$(expr $(printf "%s" "$start" | wc -c) - $endblock)"
		printf "%s" "$start" | cut -c 1-${chopoff}
		break
	else
		toformat="$toformata"
	fi
...
outhtml="<HTML><TITLE>BBC CUSTOM HOMEPAGE</TITLE><body bgcolor="black">"

outhtml="${outhtml}<h2 style=\"color:white\">Most Read</h2>$(printTag "$thehtml" "mostRead" "" | sed 's#a href="#a style="text-decoration:none;color:green;" href="https://www.bbc.co.uk#g')"
...
#readtherss
outhtml="${outhtml}<hr>$(readtherss "$therss")"


outhtml="${outhtml}<hr><h2 style=\"color:white\">Most Watched</h2>$(printTag "$thehtml" "mostWatched" "" | sed 's#a href="#a style="text-decoration:none;color:green;" href="https://www.bbc.co.uk#g')"
